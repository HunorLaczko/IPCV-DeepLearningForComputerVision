...
class NeuralNetworkBinary():
    ...
    def train(self, x_train, y_train, x_valid, y_valid, metrics, num_iterations, learning_rate, print_cost = True):
        ...
        for i in range(num_iterations):
            ...
            # training
            a_train, cache = self.forward(x_train, self.sigmoid)
            ...
            # validation
            a_valid, _ = self.forward(x_valid, self.sigmoid)
    ...
    def predict(self, x):
        ...
        a, cache = self.forward(x, self.sigmoid)
...
network = NeuralNetworkBinary(x_train.shape, 64, y_train.shape)
pretrained = False

if pretrained:
    network.loadWeights('SingleLayerWeights.npz')
else:
    history = network.train(x_train, y_train, x_valid, y_valid, [accuracy, fScore, bacc], 500, 1)
    # network.saveWeights('SingleLayerWeights.npz')
    ...
    # fig.savefig('loss_hidden.png', dpi=fig.dpi)
    plt.show()
    ...
    plt.legend()
    plt.title('Metrics')
    # fig.savefig('metric_hidden.png', dpi=fig.dpi)
    plt.show()

test_scores = network.evaluate(x_test, y_test, [accuracy, fScore, bacc])
